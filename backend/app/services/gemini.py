...
class GeminiProvider(AIProvider):
    ...
    async def generate_simulation(self, query: str, context: dict) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            f"Always map the scenario IDs (A, B, C, D) to these archetype personalities:\n"
            f"   - Scenario A: Accelerated Growth (Aggressive Leap / High Risk, High Reward)\n"
            f"   - Scenario B: Balanced Path (Sustainable & Consistent Development)\n"
            f"   - Scenario C: Defensive Hedge (Downside Risk Mitigation & Safety First)\n"
            f"   - Scenario D (if applicable): Creative Outlier (High Innovation / Unconventional Pivot)\n"
            f"2. For each scenario, outline a chronological timeline of milestones with reasonable target years (e.g. 2026, 2027, 2028, 2030), concrete pros and cons, risks, estimated costs, expected benefits, opportunity cost (what is sacrificed), and confidence level (1-100).\n"
